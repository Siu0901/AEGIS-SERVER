...
대시보드 오버레이용 좌표 스트림. **영상은 포함하지 않는다.** 마스크도 기본적으로 싣지 않는다 — 엣지 내부 계산(접지점·최근접 거리·자세 판정)에만 쓰고 결과값만 전송한다.

**예외 — 진단용 윤곽(`contour`).** 정책 `overlay_mask` 가 켜져 있을 때만 객체마다 마스크 윤곽을 함께 싣는다. 감지 결과를 눈으로 확인하기 위한 것이며 **판정에는 쓰이지 않는다** — 서버는 이 값을 읽지 않고 대시보드로 통과시키기만 한다. 기본값이 꺼짐인 이유는 대역폭이다: 객체당 24점 × 객체 수 × fps × 카메라 수만큼 좌표가 늘어난다. 상시로 켜두는 값이 아니다.
...
| `contour` | array·생략 | **진단용 마스크 윤곽.** 정규화 좌표 `[[x,y], ...]` 최대 24점. `overlay_mask` 가 꺼져 있으면 **필드를 생략한다**(`null` 을 싣지 않는다). 판정에 쓰지 않는다 |
...
| `overlay_mask` | 진단용 마스크 윤곽(`contour`)을 `frame`·`overlay` 에 싣는다(기본 `false`). 엣지가 `GET /policies` 로 받아가므로 **엣지를 재시작하지 않고** 켜고 끌 수 있다. 대역폭이 늘어나므로 확인이 끝나면 되돌린다 |
...
진단용 마스크 윤곽(`contour`, §2.1)은 **서버가 판단에 쓰지 않고 그대로 통과시킨다.**
정책 `overlay_mask` 가 꺼져 있으면 엣지가 애초에 싣지 않으므로 이 필드도 생략된다.
