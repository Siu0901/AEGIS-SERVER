...
| 무엇 | 왜 |
|---|---|
| **★ 키프레임 임베딩이 400 으로 실패했다** | `gemini-embedding-001` 은 **텍스트 전용**이다(`400 The text content is empty`). 멀티모달 임베딩(`multimodalembedding@001`)은 Vertex AI 쪽이라 AI Studio 키로 닿지 않는다. **장면을 Flash 로 묘사한 뒤 그 문장을 임베딩**하도록 바꿨다 — 역할 분리(임베딩=찾고 비교 · LLM=읽고 쓰기)는 그대로이고, 질의도 문장이라 양쪽이 같은 공간에 놓여 검색이 오히려 정확해진다. 대가는 키프레임당 호출 2회와, 묘사가 같으면 서로 다른 장면이 같은 벡터가 되는 것이다 |
| 묘사 프롬프트가 **판정을 요구하지 않는다** | 「위험해 보인다」가 나오면 그 판단이 벡터에 실려 검색 결과의 근거가 된다 — 조항을 LLM 이 만들지 못하게 한 것(FN-AI-06)과 같은 이유다 |
