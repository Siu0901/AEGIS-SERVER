...
| **★ 엣지가 카메라 RTSP 주소를 어디서 받는지 원천이 둘이다** (M9 인수인계 중 드러남) | §6 `cameras` 가 `rtsp_main` · `rtsp_sub` 를 들고 있고 `GET /cameras`(§4.5)가 그것을 내보내는데, `edge/config.yaml` 의 `streams[].rtsp_sub` 에도 같은 주소가 적혀 있다. 카메라를 교체하거나 IP 가 바뀌면 설정 화면에서 고친 값과 엣지가 보는 값이 갈리고, 그 어긋남은 **엣지가 옛 주소로 재연결을 반복하는 모습**으로만 드러난다 — 화면에는 「서브 스트림 down」으로 보여 카메라 고장과 구분되지 않는다. 카메라 표시 이름을 `GET /cameras` 하나로 모은 것과 같은 사안이며, 아래 정책 수신 경로와 **같은 통로로 함께 정해야 한다**(둘 다 엣지가 서버에서 설정을 받아오는 문제다) |
