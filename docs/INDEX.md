...
| 설정 화면 축척 안내 | 실측값 입력란 위에 5줄 — 같은 바닥 평면 · **모형 실측 cm 그대로** · **임계값 쪽에서 이미 환산됨(42.5배)** · 기준 인물 좌표 단위 일치 · 카메라 고정 |
| 캘리브레이션 「변 길이만 입력」 모드 | 네 점을 순서대로 찍고 `1→2` · `2→3` 두 변 길이만 넣으면 화면이 `(0,0) (a,0) (a,b) (0,b)` 를 만든다. 좌표 8칸을 손으로 넣던 것을 2칸으로 줄였다 — 사다리꼴이 필요한 현장을 위해 「점마다 좌표 입력」 모드는 남겨 두었다 |
| 호모그래피 적용 표시 | 「적용됨(초록) · 시각 · 지면 범위」 / 「시드 기본값(주황)」 / 「없음(빨강)」 3단. **재투영 오차 대신 지면 범위를 띄운다** — 4점은 항상 정확히 맞아떨어져 오차가 늘 0 이라 검증에 쓸 수 없다 |
| 찍은 점 번호 표시 | 클릭한 지점마다 순서 번호 뱃지. 입력표 `#` 열과 1:1 대응 (실측: 1번 = px 0.299,0.699 ↔ 뱃지 left 29.9% top 69.94%) |
...
| **§6 `cameras.rtsp_main` · `rtsp_sub` 가 조회용 기록임을 명시해 달라** | ~~원천이 둘이다~~ **정정** — 실제로 그 컬럼을 읽는 코드는 없다. REC 은 `RTSP_BASE`+규약(`recorder/config.py`), 엣지는 `edge/config.yaml`, 라이브는 브라우저가 mediamtx 로 직접 받는다. 설정 화면은 rtsp 를 **표시하지도 편집하지도 않으므로** 「화면에서 고친 값과 어긋난다」는 상황은 일어날 수 없다. 주소는 설치 시 한 번 정하고 운용 중 바꾸지 않는 값이라 로컬 설정이 맞다 — 정책값과 성격이 다르다. **§6 과 §4.5 에 「조회용 기록이며 값을 고쳐도 구독 대상은 바뀌지 않는다」를 적어 달라**(문구는 아래 「명세서에 붙일 문구」). 지금은 `PATCH /cameras` 가 그 필드를 받고 200 을 돌려주지만 아무 효과가 없다 — 화면에 입력란이 없어 아무도 부딪히지 않으나, 나중에 「카메라 주소 편집」을 넣으면 **동작하는 것처럼 보이는 입력란**이 생긴다 |
...
SSOT 명세서는 훅이 에이전트 수정을 막고 있으므로(`scripts/hooks/protect_specs.py`)
아래 문구를 사람이 붙인다. 붙이고 나면 위 표의 해당 행을 지운다.

**① `docs/AEGIS_기능명세서.md` §6 — `**cameras**` 줄 바로 아래**

```
　→ **`rtsp_main` · `rtsp_sub` 는 조회용 기록이다.** 각 구성요소는 이 값을 읽지 않고
　　 자기 로컬 설정으로 스트림을 구독한다 — REC 은 `RTSP_BASE` 환경변수, 엣지는
　　 `edge/config.yaml`, 라이브는 브라우저가 mediamtx(WHEP/HLS)로 직접 받는다.
　　 주소는 설치 시 한 번 정하고 운용 중 바꾸지 않는 값이라 각 장비의 로컬 설정에
　　 두는 편이 맞다 — 현장에서 화면으로 자주 조정하는 정책값(`policies`)과 성격이
　　 다르다. 이 컬럼은 어느 카메라가 어느 주소를 쓰는지 사람이 확인하기 위한
　　 기록이며, **값을 고쳐도 구독 대상은 바뀌지 않는다.**
```

**② `docs/AEGIS_API명세서.md` §4.5 `GET /cameras` — 필드 표에 두 줄 추가**

```
| `rtsp_main` / `rtsp_sub` | **조회용 기록이다.** 각 구성요소는 자기 로컬 설정으로 구독한다(REC=`RTSP_BASE`, 엣지=`edge/config.yaml`, 라이브=mediamtx 직결). 어느 카메라가 어느 주소를 쓰는지 확인하는 자리이며 **값을 고쳐도 구독 대상은 바뀌지 않는다** |
| `PATCH` 의 `rtsp_main`/`rtsp_sub` | 기록을 갱신할 뿐 재구독을 일으키지 않는다. 실제 주소를 바꾸려면 해당 장비의 설정을 고치고 다시 띄운다 |
```

**③ 실물 카메라 주의(선택)** — §4.5 나 §1.2 에 한 줄 있으면 M9 에서 덜 헤맨다

```
실물 IP 카메라는 벤더 고유 경로(`rtsp://192.168.0.10:554/Streaming/Channels/101` 등)를
쓰므로 개발 환경의 `{RTSP_BASE}/cam{N}/main` 규약으로 조립할 수 없다. REC 은 현재 그
규약으로 URL 을 만들고 있어 M9 에서 카메라별 주소 설정이 필요하다.
```
