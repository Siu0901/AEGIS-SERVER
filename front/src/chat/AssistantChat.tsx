 ...
 *
 * **두 화면이 같은 것을 쓴다** — 전용 화면(`AssistantPage`)과 실시간 관제 사이드.
 * 두 벌로 두면 라우팅 표시나 첨부 처리가 한쪽에만 반영돼, 같은 질문에 화면마다 다른
 * 근거가 보이게 된다.
 *
 * 관제 사이드에서는 `compact` 로 넣는다. 좁은 폭에서 예시 칩과 안내문이 대화 자체를
 * 밀어내므로 그 둘을 접는다 — 대화 로그와 입력창은 어느 쪽에서도 그대로다.
 *
