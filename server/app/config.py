...
class ServerSettings(BaseSettings):
    ...
    """「오늘」·「최근 N일」의 **날짜 경계를 정하는 시간대**.

    저장과 API 는 UTC 지만(§1.2) 집계의 「하루」는 **현장이 사는 날짜**여야 한다.
    UTC 자정으로 끊으면 한국에서 오전 9시 이전에 일어난 위반이 「오늘」에 잡히지
    않는다 — 오전 8시에 사고가 나도 개요 화면이 0 을 보여준다.

    ★ **서버 로케일을 따르지 않고 여기서 못 박는다.** 로케일을 따르면 같은 이벤트가
    어느 날에 속하는지가 서버를 어디에 두느냐에 따라 달라지고, 그 차이는 지표에
    조용히 섞인다. 원래 UTC 로 끊던 이유가 그것이었으므로 설정으로 없앤다.
    """

    site_miniature: bool = True
    """이 현장이 **미니어처 모형 시연 환경인가**(기능명세서 §4.7).

    켜면 지능 기능의 프롬프트 세 곳(챗봇 FN-AI-08 · 브리핑 09 · 심층분석 05)에
    「레고 인물 = 작업자 · 장난감 트럭 = 지게차 · 축척 42.5배」라는 사실이 실린다.

    ★ **이것을 알려주지 않으면 답이 틀린 정도가 아니라 반대가 된다.** 실측으로 모델이
    「레고 모형 피규어가 서 있다. 실제 작업자는 식별되지 않는다」고 답했는데, 같은
    프레임에서 감지 파이프라인은 그 인물들을 `person` 으로 잡아 위반을 확정하고 경고
    방송까지 내보내고 있었다. 화면과 문장이 정반대를 말하고, 사람이 믿는 쪽은 문장이다.

    실물 카메라를 실제 공장에 걸었다면 **끈다** — 그때는 이 설명이 거짓이 된다.
    감지 모델도 함께 바뀌어야 하므로(`models/weights/` 가 레고로 학습돼 있다) 이 값만
    뒤집는 것으로는 부족하다.
    """
