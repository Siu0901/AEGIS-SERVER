...
class RecSettings(BaseSettings):
    ...
    **절대경로로 정규화한다**(아래 validator). `.env` 에는 `./media/rec` 처럼 상대경로를
    ...
    rec_snapshot_fps: int = 1
    """스냅샷 버퍼 샘플링(초당 장수). 기능명세서 §4.4 = 1.

    `GET /status` 의 `recording.snapshot_fps` 로 서버에 보고된다.
    """

    rec_snapshot_window_s: int = 60
    """스냅샷 버퍼 보관 구간(초). 기능명세서 §4.4 = 60.

    이 안의 시각은 `GET /keyframe` 이 메모리에서 즉시 답하고, 그보다 오래된 시각만
    세그먼트에서 추출한다.
    """

    rec_snapshot_keyframes_only: bool = False
    """스냅샷을 **키프레임만 디코딩**해 뽑는가. 기본은 끔(명세서 동작).

    §4.4 는 「초당 1장 인코딩은 리먹스 부하에 비해 무시할 수준」이라고 적었는데, 그
    전제는 **디코딩이 하드웨어(NVDEC)로 공짜에 가깝다**는 것이다. 소프트웨어 디코딩을
    쓰는 개발 노트북에서는 그렇지 않다 — 1080p 스트림 한 대를 초당 1장으로 샘플링하는
    데 코어의 33% 가 든다(실측 20초에 CPU 6.5초). 인코딩이 아니라 **매 프레임 디코딩**이
    비용이다.

    이 값을 켜면 디코더가 키프레임만 풀어 **코어 7%**(20초에 1.4초)로 내려간다. 대신
    샘플 간격이 스트림의 GOP 를 따르므로, GOP 가 1초보다 길면 `rec_snapshot_fps` 를
    채우지 못하고 확정 시각과의 오차가 §4.4 의 0.5초를 넘을 수 있다.

    **그래서 기본값은 끔이다.** 명세서가 보장한 샘플 간격을 설정 하나로 조용히 바꾸지
    않는다(CLAUDE.md 절대규칙 8). 젯슨은 NVDEC 이 있으므로 켤 이유가 없고, 개발 기계에서
    CPU 가 모자랄 때만 켠다.
    """
