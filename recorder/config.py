...
class RecSettings(BaseSettings):
    ...
    **절대경로로 정규화한다**(아래 validator). `.env` 에는 `./media/rec` 처럼 상대경로를
    ...
    이 구간의 **H.264 바이트를 그대로** 메모리에 들고 있는다(2.5 Mbps 기준 카메라당
    약 19MB). 이 안의 시각은 `GET /keyframe` 이 그 자리에서 1프레임만 디코딩해 답하고,
    그보다 오래된 시각만 세그먼트 파일에서 추출한다.

    **샘플링 주기가 없다.** 예전에는 초당 1장을 뽑아 두었는데, 그러려면 매 프레임을
    디코딩해야 해서 1080p 두 대에 코어 66% 가 들었다(실측). 지속 비용을 사건 시점의
    일회성 비용으로 옮기면서 근사 없이 요청한 시각의 프레임을 낼 수 있게 됐다.
    """
